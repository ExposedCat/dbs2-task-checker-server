...
export async function readDataset<F extends 'txt' | 'json'>(
	args: GetDatasetArgs<F>
): Promise<GetDatasetResponse<F>> {
	...
	try {
		const raw = await Bun.file(
			`${process.cwd()}/datasets/${trimmed}/dataset.txt`
		).text()
...
export async function extractDatasetKinds({
	datasetId,
	database
}: ExtractDatasetKindsArgs) {
...
export type DeleteDatasetKindArgs = {
	id: string
	database: Database
	kind: string
}

export async function deleteDatasetKind({
	id,
	database,
	kind
}: DeleteDatasetKindArgs) {
	await database.datasets.updateOne({ id }, { $pull: { bank: { kind } } })
